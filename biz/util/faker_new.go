...
func InsertQuizData(cfg *config.Config, mongo *mongodb.Mongodb) {
	...
	for i := 0; i < 10000; i++ {
		...
		// _, err := mongo.FakerConn.Collection("participant").InsertOne(context.Background(), user)

		if err != nil {
			zap.L().Fatal("mongo.FakerConn.Collection(participant).InsertOne (InsertQuizData)", zap.Error(err))
		}
	}

	var allQuizParticipants = make([]domain.Participant, 20*150) // 150 quiz pertama diikuti 20 user
	...
	for i := 0; i < 20*200; i++ {
		...
		if i%20 == 0 && i < 3000 {
			...
			for k := participantIdx; k < participantIdx+20; k++ {
				// fmt.Println(k)
				allQuizParticipants[k] = quizParticipants[loopIdx] // masukin 20 participant untuk setiap quiz
				loopIdx++
			}
			participantIdx += 20
		}
		...
		if i < 3000 {
			...
			for j := 0; j < 20; j++ {
				...
				userAnswers[j] = domain.UserAnswer{
					ChoiceID:      userChoice.ID.Hex(),
					ParticipantID: quizParticipants[j].UserID,
		...
		// _, err := mongo.FakerConn.Collection("question").InsertOne(context.Background(), quizQuestion)

		if err != nil {
			zap.L().Fatal("mongo.FakerConn.Collection(question).InsertOne (InsertQuizData) (util)", zap.Error(err))
		}
	}
	...
	first = 0
	for i := 0; i < 200; i++ {
		var thisQuizParticipant []domain.Participant
		var thisQuizQuestions = make([]domain.Question, 20)
		...
		for k := first; k < first+20; k++ {
			thisQuizQuestions[thisQuestionIdx] = quizQuestions[k]
			thisQuestionIdx++
		}
		...
		_, err := mongo.FakerConn.Collection("base_quiz").InsertOne(context.Background(), quiz)
		if err != nil {
			zap.L().Error("mongo.FakerConn.Collection(quiz).InsertOne (InsertQuizData)", zap.Error(err))
		}
	}
